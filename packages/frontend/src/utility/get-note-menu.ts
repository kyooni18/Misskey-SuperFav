...
export function getNoteMenu(props: {
	...
	function toggleSuperFavorite(superFavorite: boolean): void {
		os.apiWithDialog((superFavorite ? 'notes/super-favorites/create' : 'notes/super-favorites/delete') as keyof Misskey.Endpoints, {
			noteId: appearNote.id,
		});
	}
	...
	if ($i) {
		...
		menuItems.push(statePromise.then(state => (state as { isSuperFavorited?: boolean }).isSuperFavorited ? {
			icon: 'ti ti-star-off',
			text: '슈퍼 즐겨찾기 해제',
			action: () => toggleSuperFavorite(false),
		} : {
			icon: 'ti ti-stars',
			text: '슈퍼 즐겨찾기',
			action: () => toggleSuperFavorite(true),
		}));
